...
namespace Aprismatic
{
    public static partial class BigIntegerExt
    {
        ...
        public static BigInteger GenPseudoPrime100k(this BigInteger T, int bits, int confidence, RandomNumberGenerator rng)
        {
            if (bits < 2) throw new ArgumentOutOfRangeException(nameof(bits), bits, "GenPseudoPrime can only generate prime numbers of 2 bits or more");
            ...
            do
            {
                result = BigInteger.Zero.GenRandomBits(bits, rng);
                result |= BigInteger.One; // make it odd
            } while (!result.IsProbablePrime100k(confidence, rng));

            return result;
        }

        public static BigInteger GenPseudoPrime500k(this BigInteger T, int bits, int confidence, RandomNumberGenerator rng)
        {
            if (bits < 2) throw new ArgumentOutOfRangeException(nameof(bits), bits, "GenPseudoPrime can only generate prime numbers of 2 bits or more");
            ...
            do
            {
                result = BigInteger.Zero.GenRandomBits(bits, rng);
                result |= BigInteger.One; // make it odd
            } while (!result.IsProbablePrime50k(confidence, rng));

            return result;
        }

        public static BigInteger GenPseudoPrime1M(this BigInteger T, int bits, int confidence, RandomNumberGenerator rng)
        {
            if (bits < 2) throw new ArgumentOutOfRangeException(nameof(bits), bits, "GenPseudoPrime can only generate prime numbers of 2 bits or more");
            ...
            do
            {
                result = BigInteger.Zero.GenRandomBits(bits, rng);
                result |= BigInteger.One; // make it odd
            } while (!result.IsProbablePrime200k(confidence, rng));
        ...
        public static bool IsProbablePrime100k(this BigInteger T, int confidence, RandomNumberGenerator rng)
        {
            var thisVal = BigInteger.Abs(T);
            if (thisVal.IsZero || thisVal.IsOne) return false;
            ...
            if (thisVal <= UInt64.MaxValue)
            {
                ...
                foreach (var smallPrime in PrimesBelow100k)
                {
                    if (smallPrime >= uival)
                        return true;
            ...
            {
                foreach (var smallPrime in PrimesBelow100k_BI)
                {
                    if ((thisVal % smallPrime).IsZero)
                        return false;
                }
        ...
        public static bool IsProbablePrime50k(this BigInteger T, int confidence, RandomNumberGenerator rng)
        {
            var thisVal = BigInteger.Abs(T);
            if (thisVal.IsZero || thisVal.IsOne) return false;
            ...
            if (thisVal <= UInt64.MaxValue)
            {
                ...
                foreach (var smallPrime in PrimesBelow50k)
                {
                    if (smallPrime >= uival)
                        return true;
            ...
            {
                foreach (var smallPrime in PrimesBelow50k_BI)
                {
                    if ((thisVal % smallPrime).IsZero)
                        return false;
                }
        ...
        public static bool IsProbablePrime200k(this BigInteger T, int confidence, RandomNumberGenerator rng)
        {
            var thisVal = BigInteger.Abs(T);
            if (thisVal.IsZero || thisVal.IsOne) return false;
            ...
            if (thisVal <= UInt64.MaxValue)
            {
                ...
                foreach (var smallPrime in PrimesBelow200k)
                {
                    if (smallPrime >= uival)
                        return true;
            ...
            {
                foreach (var smallPrime in PrimesBelow200k_BI)
                {
                    if ((thisVal % smallPrime).IsZero)
                        return false;
                }
